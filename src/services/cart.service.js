...
const getCartByUser = async (user) => {
  // console.log(user);
 // const cartData = await Cart.findById(user._id);
  // const products = await Product.findById(user._id);
...
const addProductToCart = async (user, productId, quantity) => {
  ...
  // const index = await user.Cart.findIndex(() => productId === user.Cart.productId);
 ...
 // const index = await cart.cartItems.findIndex((item) => item.product._id === productId);
  
  for (let item = 0; item < cart.cartItems.length; item++){
   // console.log(cart.cartItems[item].product._id);
    //console.log(productId);
    if (cart.cartItems[item].product._id == productId) {
     // console.log("hey");
